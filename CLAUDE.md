...
fonts/          nanum-myeongjo.woff2(본문) + song-myung.woff2(디스플레이 세리프)
                + diphylleia-title.woff2(제목 한글+영문 공통) — 모두 OFL 서브셋
...
- **타이포 시스템(3역할)**: **디스플레이 `--display` = 송명(Song Myung, OFL, `fonts/song-myung.woff2` 서브셋 40KB)** — 제목·섹션헤딩·구슬 글리프·눈금 등급명·문서 헤딩에 적용(고대비 천체 알마낙 톤). **본문 `--serif` = 나눔명조**(긴 콘텐츠 가독). **유틸리티 `--mono` = 시스템 모노**(아이브로·주사위 눈·확률). **메인 제목 `.app-title`만 장식 서체 = Diphylleia**(OFL, 한글+Latin 공통 → 한/영 동일 서체). 단일 굵기(400)라 '굵게'는 `-webkit-text-stroke: 1.4px var(--gold)` + `paint-order: stroke fill`로 합성. **제목 글자만 서브셋**(선택 도우미/Choice Helper) — 앱 이름 변경 시 재서브셋 필요. 송명 서브셋은 앱 전체 한글+Latin 기준이라 표시 글리프 누락 없음(새 한글 카피 추가 시 재생성).
...
10. **메인 제목 장식 서체**: (초안 Lobster+나눔붓 →) **Diphylleia**로 한/영 통일, text-stroke로 굵게, `.app-title`에만 크게(OFL, 제목 글자 서브셋).
